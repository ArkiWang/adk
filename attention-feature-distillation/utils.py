...
def train_kl(module_list, optimizer, criterion, train_loader, device, args):
    ...
    criterion_kl = criterion[1]
    criterion_kd = criterion[2]
    criterion_kd_h = criterion[3]
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        inputs, targets = inputs.to(device), targets.to(device)
        with torch.no_grad():
            feat_t, output_t = model_t(inputs, is_feat=True)
            feat_t = [f.detach() for f in feat_t]
            feat_h, output_h = model_h(inputs, is_feat=True)
            feat_h = [f.detach() for f in feat_h]
        ...
        if args.is_hint:
          loss_kd_h = criterion_kd_h(feat_s, feat_h)
          loss = loss_ce + args.alpha * loss_kl + args.beta * (0.7*loss_kd + 0.3*loss_kd_h)
