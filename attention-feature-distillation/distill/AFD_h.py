...
class AFD_h(nn.Module):
    def __init__(self, args):
        super(AFD_h, self).__init__()
        self.guide_layers = args.guide_layers_h
...
class Attention(nn.Module):
    def __init__(self, args):
        ...
        self.p_t = nn.Parameter(torch.Tensor(len(args.h_shapes), args.qk_dim))
...
class LinearTransformTeacher(nn.Module):
    def __init__(self, args):
        ...
        self.query_layer = nn.ModuleList([nn_bn_relu(t_shape[1], args.qk_dim) for t_shape in args.h_shapes])
...
class LinearTransformStudent(nn.Module):
    def __init__(self, args):
        ...
        self.t = len(args.h_shapes)
        self.s = len(args.s_shapes)
        self.qk_dim = args.qk_dim
        self.relu = nn.ReLU(inplace=False)
        self.samplers = nn.ModuleList([Sample(t_shape) for t_shape in args.unique_h_shapes])

        self.key_layer = nn.ModuleList([nn_bn_relu(s_shape[1], self.qk_dim) for s_shape in args.s_shapes])
        self.bilinear = nn_bn_relu(args.qk_dim, args.qk_dim * len(args.h_shapes))
